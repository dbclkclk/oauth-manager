 ...
 * @since  1.6
 */
class TeachproModelStudentsForm extends JModelForm
{
	private $item = null;
	...
	protected function populateState()
	{
		...
		{
			$id = JFactory::getApplication()->getUserState('com_teachpro.edit.students.id');
		}
		else
		{
			$id = JFactory::getApplication()->input->get('id');
			JFactory::getApplication()->setUserState('com_teachpro.edit.students.id', $id);
		}

		$this->setState('students.id', $id);
		...
		{
			$this->setState('students.id', $params_array['item_id']);
		}
	 ...
	 */
	public function getTable($type = 'Students', $prefix = 'TeachproTable', $config = array())
	{
	...
	{
		// Get the id.
		$id = (!empty($id)) ? $id : (int) $this->getState('students.id');
	...
	{
		// Get the form.
		$form = $this->loadForm('com_teachpro.students', 'studentsform', array(
			'control'   => 'jform',
			'load_data' => $loadData
			)
		);
	...
	{
		$data = JFactory::getApplication()->getUserState('com_teachpro.edit.students.data', array());
	...
	{
		$id    = (!empty($data['id'])) ? $data['id'] : (int) $this->getState('students.id');
