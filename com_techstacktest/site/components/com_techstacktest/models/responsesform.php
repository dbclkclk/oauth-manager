...
class TeachproModelResponsesForm extends JModelForm
{
	...
	{
		// Get the form.
		$form = $this->loadForm('com_teachpro.responses', 'testsform', array(
			'control'   => 'jform',
			'load_data' => $loadData
			)
		);
	...
	{
		$data = JFactory::getApplication()->getUserState('com_teachpro.edit.responses.data', array());
	...
	{
		$id    = (!empty($data['id'])) ? $data['id'] : (int) $this->getState('responses.id');
