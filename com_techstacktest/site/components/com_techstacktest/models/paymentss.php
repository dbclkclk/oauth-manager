 ...
 * @since  1.6
 */
class TeachproModelPaymentss extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	protected function getListQuery()
	{
		...
		// Select the required fields from the table.
                if($this->getState("total_payment"))
                {
                    
                     $query->select("sum(a.amount_recieved) as total_payment");
                }
                else
                {
                    $query
		...
		$query->from('`#__teachpro_payment` AS a');
	...
	private function isValidDate($date)
	{
        ...
        public function getTotalPaymentByPaymentTerms($paymenttermsid)
        {
            $db = JFactory::getDbo();
            
            $this->setState("total_payment", true);
            $query = $this->getListQuery();
            
            
           
            
            $query->where("a.paymenttermsid=".$paymenttermsid);
