 ...
 * @since  1.6
 */
class TeachproModelSpouseForm extends JModelForm
{
	private $item = null;
	...
	protected function populateState()
	{
		...
		{
			$id = JFactory::getApplication()->getUserState('com_teachpro.edit.spouse.id');
		}
		else
		{
			$id = JFactory::getApplication()->input->get('id');
			JFactory::getApplication()->setUserState('com_teachpro.edit.spouse.id', $id);
		}

		$this->setState('spouse.id', $id);
		...
		{
			$this->setState('spouse.id', $params_array['item_id']);
		}
	 ...
	 */
	public function getTable($type = 'Spouse', $prefix = 'TeachproTable', $config = array())
	{
	...
	{
		// Get the id.
		$id = (!empty($id)) ? $id : (int) $this->getState('spouse.id');
	...
	{
		// Get the form.
                // Second parameter is the xml.
		$form = $this->loadForm('com_teachpro.spouse', 'enrollment', array(
			'control'   => 'jform',
			'load_data' => $loadData
			)
		);
	...
	{
		$data = JFactory::getApplication()->getUserState('com_teachpro.edit.spouse.data', array());
	...
	{
		$id    = (!empty($data['id'])) ? $data['id'] : (int) $this->getState('spouse.id');
