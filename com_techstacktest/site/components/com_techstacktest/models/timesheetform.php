 ...
 * @since  1.6
 */
class TeachproModelTimesheetForm extends JModelForm
{
	private $item = null;
	...
	{
		// Get the form.
		$form = $this->loadForm('com_teachpro.timesheet', 'timesheet', array(
			'control'   => 'jform',
			'load_data' => $loadData
			)
		);
	...
	{
		$data = JFactory::getApplication()->getUserState('com_teachpro.edit.timesheet.data', array());
	...
	{
		$id    = (!empty($data['id'])) ? $data['id'] : (int) $this->getState('timesheet.id');
